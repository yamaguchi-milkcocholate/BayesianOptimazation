...
for i in range(5):
    ...
    f = GaussianMixtureFunction(dim=dim, mean_1=2, mean_2=3)
    method = SelectAcquisitionDiff(f=f, domain=domain, fill_in_strategy=fill_in_strategy, maximize=True, eta=1/dim)
    method.run_optimization(max_iter=500, eps=0)

    dim = len(domain)
    fill_in_strategy = 'mix'
    f = GaussianMixtureFunction(dim=dim, mean_1=2, mean_2=3)
    method = SelectAcquisitionDiff(f=f, domain=domain, fill_in_strategy=fill_in_strategy, maximize=True, mix=0.5, eta=1/dim)
    method.run_optimization(max_iter=500, eps=0)
